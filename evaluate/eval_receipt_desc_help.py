...
def ocr_detect(img_path):
        ...
        for line in out:
            if line:
                for entry in line:
                    ...
                    cent_x,cent_y=(np.array(vertices[2])+np.array(vertices[0]))//2
                    w,h= np.array(vertices[2])-np.array(vertices[0])
                    cent_x,cent_y,w,h=int(cent_x),int(cent_y),int(w),int(h)
                    json_record.append({"text":entry_text, "score": entry[1][1], "position": [cent_x, cent_y,w,h],"distance":0})
                    if entry_text:
    ...
    return text, img_anno, json_record
...
with open(markdown_file_path, 'w') as md_file:
    for filename in tqdm(sorted(os.listdir(image_directory))):
        if filename.endswith(".jpg"):
            ...
            ocr_text, img_anno, json_record = ocr_detect(img_path)
            with open(os.path.join("./test_data/ocr_detect_texts", filename.replace(".jpg", ".json")), "w", encoding="utf-8") as f:
                json.dump({"text": ocr_text}, f, ensure_ascii=False, indent=2)
